...
```bash
git init
git remote add origin https://github.com/blyons-dev/BOMB.git
```
...
```
3a. Optional. Verify you're on the right branch.
```bash
git branch -v
```
4. The plugin in KiCad can only be accessed in the Layout Editor, so navigate there. The plugin will automatically detect any Unstaged Changes that you have made, you just need to verify that by clicking **Rescan**.
5. Click the **Stage Changed** button to prepare everything for a commit
6. Write in the Commit Message a description of what you changed and click the **Sign Off** button as well.
7. Click **Commit** and then **Push** to send everything to the online repo.
